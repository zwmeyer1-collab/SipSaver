...
const outputPath = path.join(outputDir, "tampa-raw.json");
...
async function scrapeVenue(venue, verbose) {
  ...
  for (const initialTarget of crawlTargets) {
    ...
    try {
      ...
      if (verbose) {
        console.log(`[scrape] ${venue.name} homepage ${firstPage.status} ${firstPage.finalUrl}`);
      }
      ...
      for (const childLink of childLinks) {
        ...
        try {
          ...
          if (verbose) {
            console.log(`[scrape] ${venue.name} child ${childPage.status} ${childPage.finalUrl}`);
          }
        } catch (error) {
          errors.push({
            url: childLink,
...
async function main() {
  ...
  const seedVenues = JSON.parse(seedFile);
  const venues = options.limit ? seedVenues.slice(0, options.limit) : seedVenues;
